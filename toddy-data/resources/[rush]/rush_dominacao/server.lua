...
rsh = {}
Tunnel.bindInterface("rush_dominacao",rsh)
...
function rsh.pagamento(perm,blip,k)
    ...
    if user_id then 
        ...
        if perm == "drogas.permissao" and blip == 1 then
            if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("metanfetamina")*qtdDrogas <= vRP.getInventoryMaxWeight(user_id) then
                locais[k].timer = 7200
        ...
        elseif perm == "drogas.permissao" and blip == 3 then
            if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("lsd")*qtdDrogas <= vRP.getInventoryMaxWeight(user_id) then
                locais[k].timer = 7200
                vRP.giveInventoryItem(user_id, "metanfetamina", qtdDrogas)
            else
                TriggerClientEvent("Notify", source, "negado","Mochila Cheia!")
            end
