...
emP = Tunnel.getInterface("rize_radio")
-----------------------------------------------------------------------------------------------------------------------------------------
-- FUNCTION
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterNUICallback("ButtonClick",function(data,cb)
	...
	elseif data == "mafiaservia" then
		if emP.checkPermission2("servia.permissao","Bratva") then
			outServers()
			exports.tokovoip_script:addPlayerToRadio(1029)
		end
	elseif data == "mafiaalbanesa" then
		if emP.checkPermission2("albanesa.permissao","Triade") then
			outServers()
	...
	elseif data == "lifeinvader" then
		if emP.checkPermission2("life.permissao","LifeInvader") then
			outServers()
			exports.tokovoip_script:addPlayerToRadio(1030)
		end
...
RegisterCommand("radio",function(source,args)
	if GetEntityHealth(PlayerPedId()) > 101 then
		...
		TriggerEvent("Notify","negado","Negado","Você não pode fazer isso em coma.")	
    end
end)
		...
		if parseInt(args[1]) < 1017 then
        	if emP.checkRadio() then
				if emP.checkPermission() then
					...
					TriggerEvent("Notify","sucesso","Sucesso","Você entrou na Frequência <b>"..args[1].."</b> do rádio.",8000)
				end
			end
		else
			TriggerEvent("Notify","negado","Negado","Você não tem permissão.")
		end
	end
else
	TriggerEvent("Notify","negado","Negado","Você não pode fazer isso em coma.")	
...
RegisterCommand("radiod",function(source,args)
    if emP.checkRadio() then
		if emP.checkPermission() then
			outServers()
		    TriggerEvent("Notify","sucesso","Sucesso","Você desconectou de todos os canais.")
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
