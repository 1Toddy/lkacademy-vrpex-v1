...
emP = {}
Tunnel.bindInterface("rize_corrida",emP)
-----------------------------------------------------------------------------------------------------------------------------------------
-- FUNÇÕES
-----------------------------------------------------------------------------------------------------------------------------------------
local pay = {
	[1] = { ['min'] = 20000, ['max'] = 30000 }
}
...
function emP.checkPolice()
	...
	if user_id then
		local policia = vRP.getUsersByPermission("dpla.permissao")
		if #policia < 6 then
			TriggerClientEvent("Notify",source,"aviso","Negado","Número insuficiente de policiais no momento.",8000)
			return false
		end
	end
	return true
...
function emP.CorridaLiberada()
	...
	if user_id then
		if vRP.getInventoryItemAmount(user_id,"gps") >= 1 then
			...
			TriggerClientEvent("Notify",source,"negado","Negado","Você não tem GPS modificado para corrida na sua mochila.")
		end
	end
end
...
function emP.checkPermission()
	...
	return not (vRP.hasPermission(user_id,"dpla.permissao") or vRP.hasPermission(user_id,"medico.permissao"))
end
