...
$(document).ready(function() {
    document.onkeydown = function(data) {
        if (data.keyCode == 27) {
            ...
            $('#total').html('0'); 
            change = {};
            $.post('http://rize_skinshop/reset', JSON.stringify({}))
            $.post('http://rize_skinshop/fechar', JSON.stringify({ id: false }))
        }
    }

    $("#leftHeading").click(function() {
        $.post('http://rize_skinshop/leftHeading', JSON.stringify({ value: 10 }));
    })

    $("#handsUp").click(function() {
        $.post('http://rize_skinshop/handsUp', JSON.stringify({}));
    })

    $("#rightHeading").click(function() {
        $.post('http://rize_skinshop/rightHeading', JSON.stringify({ value: 10 }));
    })

    $("#payament").click(function() {
        $(".loja-de-roupa").fadeOut()
        $.post('http://rize_skinshop/fechar', JSON.stringify({ id: false }));
        $.post('http://rize_skinshop/payament', JSON.stringify({ price: $('#total').text() }));
        $('#total').html('0');
        change = {};
    })
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.openLojaRoupa) {
            ...
            $.post('http://rize_skinshop/fechar', JSON.stringify({ id: true }))
...
function selectPart(element) {
    ...
    $.post('http://rize_skinshop/changePart', JSON.stringify({ part: dataPart }))
}
...
function select(element) {
    ...
    $.post('http://rize_skinshop/changeCustom', JSON.stringify({ type: dataPart, id: roupaID }));
}

$(".fa-angle-right").click(function() {
    $.post('http://rize_skinshop/changeColor', JSON.stringify({ type: dataPart, id: roupaID, action: "mais" }));
})

$(".fa-angle-left").click(function() {
    $.post('http://rize_skinshop/changeColor', JSON.stringify({ type: dataPart, id: roupaID, action: "menos" }));
})
