...
function dragAndDrop() {
  ...
  $(".trunk-workspace .objects").droppable({
    ...
    drop: function(event, ui) {
      ...
      $.post("http://rize_chest/storeItem",
        JSON.stringify({
          item: el_key,
          el_amount
        })
...
$(document).ready(function() {
  ...
  document.onkeyup = function(data) {
    if (data.which == 27) {
      $.post("http://rize_chest/chestClose", JSON.stringify({}), function(datab) {});
    }
  };
});
...
var templateTrunkChest = (key, amount, image, name, weight, target) => {
  ...
  <div class="cell" data-item-key="${key}" data-item-amount="${amount}">
    ...
    <div class="row">
      <div class="image" style="background-image: url(nui://rize_inventory/nui/images/${image}.png)"></div>
    </div>
    <div class="row">
...
const updateChest = () => {
  ...
  $.post("http://rize_chest/requestChest", JSON.stringify({}), data => {
