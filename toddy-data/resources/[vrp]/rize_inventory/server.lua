...
Tunnel.bindInterface("rize_inventory",cRP)
vCLIENT = Tunnel.getInterface("rize_inventory")
vRPclient = Tunnel.getInterface("vRP")

vGARAGE = Tunnel.getInterface("rize_garages")
...
local webhookenviaritem = "https://discord.com/api/webhooks/790114960574775296/ZOzasDFhGsYYXnYwEnaLuVsHbUFfI1Z5TwN1NBBmyNsZtHk1_Hzt0-bsq3xMcTT02l96"
			...
			if parseInt(amount) > 0 then
				if vRP.getInventoryWeight(nuser_id) + vRP.getItemWeight(itemName) * amount <= vRP.getInventoryMaxWeight(nuser_id) then
					if vRP.tryGetInventoryItem(user_id,itemName,amount) then
						...
						TriggerClientEvent("Notify",source,"sucesso","Sucesso","Enviou <b>"..vRP.format(parseInt(amount)).."x "..vRP.itemNameList(itemName).."</b>.",8000)
						...
						TriggerClientEvent("Notify",nplayer,"sucesso","Sucesso","Recebeu <b>"..vRP.format(parseInt(amount)).."x "..vRP.itemNameList(itemName).."</b>.",8000)
		...
		if type == "usar" then
					...
					if bandagem[user_id] == 0 or not bandagem[user_id] then
						if vRP.tryGetInventoryItem(user_id,"bandagem",1) then
							...
							SetTimeout(9000,function()
								...
								TriggerClientEvent("Notify",source,"sucesso","Sucesso","<b>Bandagem</b> utilizada com sucesso.",8000)
							end)
						end
					else
						TriggerClientEvent("Notify",source,"importante","Importante","Você precisa aguardar <b>"..bandagem[user_id].." segundos</b> para utilizar outra Bandagem.",8000)
					end
				else
						...
						if bandagem[user_id] == 0 or not bandagem[user_id] then
							if vRP.tryGetInventoryItem(user_id,"chocolate",1) then
								...
								SetTimeout(7000,function()
									...
									vRPclient._DeletarObjeto(source)
									TriggerClientEvent('cancelando',source,false)
									TriggerClientEvent("Notify",source,"sucesso","Sucesso","<b>Chocolate</b> utilizada com sucesso.",8000)
								end)
							end
						else
							TriggerClientEvent("Notify",source,"importante","Importante","Você não pode comer muito rapido.",8000)
						end
					else
						TriggerClientEvent("Notify",source,"aviso","Aviso","Você não pode utilizar de vida cheia ou nocauteado.",8000)
					end
			...
			elseif itemName == "mochila" then
				if vRP.getInventoryMaxWeight(user_id) >= 90 then
					TriggerClientEvent("Notify",source,"negado","Negado","Você não pode equipar mais mochilas.",8000)
				else
					if vRP.tryGetInventoryItem(user_id,"mochila",1) then
						TriggerClientEvent('Creative:Update',source,'updateMochila')
						vRP.varyExp(user_id,"physical","strength",650)
						TriggerClientEvent("Notify",source,"sucesso","Sucesso","<b>Mochila</b> equipada com sucesso.",8000)
					end
				end
			elseif itemName == "cerveja" then
				if vRP.tryGetInventoryItem(user_id,"cerveja",1) then
					...
					SetTimeout(30000,function()
						...
						TriggerClientEvent('cancelando',source,false)
						vRPclient._DeletarObjeto(source)
						TriggerClientEvent("Notify",source,"sucesso","Sucesso","<b>Cerveja</b> utilizada com sucesso.",8000)
					end)
				end
			...
			elseif itemName == "energetico" then
				if vRP.tryGetInventoryItem(user_id,"energetico",1) then
					...
					SetTimeout(7000,function()
						...
						TriggerClientEvent('cancelando',source,false)
						vRPclient._DeletarObjeto(source)
						TriggerClientEvent("Notify",source,"sucesso","Sucesso","<b>Energético</b> utilizado com sucesso.",8000)
					end)
					SetTimeout(60000,function()
						TriggerClientEvent('energeticos',source,false)
						TriggerClientEvent("Notify",source,"aviso","Aviso","O efeito do <b>Energético</b> passou e o coração voltou a bater normalmente.",8000)
					end)
				end
	...
	if user_id then
		if vRP.getExp(user_id,"physical","strength") == 1900 then -- 90Kg
			if vRP.getInventoryMaxWeight(user_id)-vRP.getInventoryWeight(user_id) >= 15 then
				TriggerClientEvent("Notify",source,"sucesso","Sucesso","Você desequipou uma de suas mochilas.")
				vRP.varyExp(user_id,"physical","strength",-580)
				vRP.giveInventoryItem(user_id,"mochila",1)
			else
				TriggerClientEvent("Notify",source,"negado","Negado","Você precisa esvaziar a mochila antes de fazer isso.")
			end
		...
		elseif vRP.getExp(user_id,"physical","strength") == 670 then -- 51Kg
			if vRP.getInventoryMaxWeight(user_id)-vRP.getInventoryWeight(user_id) >= 45 then
				TriggerClientEvent("Notify",source,"sucesso","Sucesso","Você desequipou sua mochila.")
				vRP.varyExp(user_id,"physical","strength",-650)
				vRP.giveInventoryItem(user_id,"mochila",1)
			else
				TriggerClientEvent("Notify",source,"negado","Negado","Você precisa esvaziar a mochila antes de fazer isso.")
		...
		elseif vRP.getExp(user_id,"physical","strength") == 20 then -- 6Kg
			TriggerClientEvent("Notify",source,"negado","Negado","Você não tem mochilas equipadas.")
		end
	end
end)