...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local logEntrada = "https://discord.com/api/webhooks/847246443847614485/ZMqE5sm2201Aw5jdsAqxcLnRfHqmPOugttm4lbN036OKMaefJKyyIHwlLuLpLQ6-rkpB"
local logSaida = "https://discord.com/api/webhooks/847246443847614485/ZMqE5sm2201Aw5jdsAqxcLnRfHqmPOugttm4lbN036OKMaefJKyyIHwlLuLpLQ6-rkpB"
local policiaPonto = ""
local resgatePonto = ""
-----------------------------------------------------------------------------------------------------------------------------------------
-- BASE.LUA
			...
			PerformHttpRequest(logSaida, function(err, text, headers) end, 'POST', json.encode({
				embeds = {
					{ 
						...
						thumbnail = {
							url = "https://cdn.discordapp.com/attachments/847159048611823626/851516372928299018/LK.png"
						}, 
						fields = {
							{ 
								name = "**ID:**", 
						...
						footer = { 
							...
							icon_url = "https://cdn.discordapp.com/attachments/847159048611823626/851516372928299018/LK.png" 
						},
						color = 15914080 
					}
				}
			...
			elseif vRP.hasGroup(user_id,"dmla") then
				...
				PerformHttpRequest(resgatePonto, function(err, text, headers) end, 'POST', json.encode({
					embeds = {
						{ 
							title = identity.name.." saiu de serviço.",
							description = "Registro de Ponto do Departamento Médico. Registro de saída de serviço.\n⠀",
							thumbnail = {
								url = "https://cdn.discordapp.com/attachments/847159048611823626/851516372928299018/LK.png"
							},
							fields = {
			...
			if not vRP.isBanned(user_id) then
				...
				if vRP.isWhitelisted(user_id) then
					if vRP.rusers[user_id] == nil then
						deferrals.update("Carregando as informações")
					...
					deferrals.done("Você não tem whitelist. Entre no nosso discord. discord.gg/mcswGfmzh4 [ ID: "..user_id.." ]")
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
			else
				deferrals.done("Você foi engui$$ado pela STAFF [ Mais informações em: discord.gg/mcswGfmzh4 ] ID: "..user_id.." ")
				--print(user_id)
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
